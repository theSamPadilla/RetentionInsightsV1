...
#
# grab all inactive users for the supplied study id
#

result = c.execute( 'SELECT * FROM surveys_user WHERE studyID_id=? AND active_p=false' , ( args.studyid , ) )
...
for user in user_records:
    ...
    try:
        message = twilio_client.messages.create(
            to='+1'+user[2] ,
            from_='+16072282179',
            body='What\'s up, ' + user[1] + '? Thank you for agreeing to participate in the Morningside student well-being project. Please confirm your participation by following the supplied link at http://'+BASE_URL+token+'. Please complete your confirmation on or before ' + dt_to_human( end_dt ) + '.  Free drinks!' )
